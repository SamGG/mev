...
public class SlideData extends Vector implements ISlideData, ISlideMetaData, java.io.Serializable {
    ...
    public void applyRatioStatistics(Properties properties) {
        ...
        try{
            ...
            cw = ((new RatioStats(cw, true, confInt)).getRatioStatsColumnWorker());
            setNormalizedIntensities(cw, goodValues);
            normalizedState = ISlideData.LINEAR_REGRESSION;
        } catch (Exception e) {
            javax.swing.JOptionPane.showMessageDialog(new javax.swing.JFrame(), "Error Performing Normalization: Data Unchanged", "Normalization Error : Aborted", javax.swing.JOptionPane.WARNING_MESSAGE);
    ...
    public void applyLowess(int bins) {
        ...
        double[] logProduct = new double[size()];
        ...
        for (int i = 0; i < size(); i++) {
            ...
            logRatios[i] = Xcon.log2(sdes[i].getRatio(ISlideDataElement.CY5, ISlideDataElement.CY3, AC.LINEAR));
            logProduct[i] = Xcon.log10(sdes[i].getIntensity(ISlideDataElement.CY3) * sdes[i].getIntensity(ISlideDataElement.CY5));
        }
