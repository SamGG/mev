...
public class ReadGeneSet {
	...
	/*	System.out.println("Printing Gene sets. in read_TXTFile function......................");
		for(int i=0; i<set[0].getAllGenesetNames().size(); i++){
			System.out.println("Gene set is:"+set[0].getGeneSetName(i));
			for(int j=0; j<set[i].getGenesinGeneset().size(); j++){
				System.out.println("Gene in gene set is:"+(String)set[i].getGenesinGeneset().get(j));
    ...
    */ 
    ...
    public FloatMatrix createAssociationMatrix(Geneset[]gSets, Vector unique_genes_in_dataset, int min_genes){
    	...
    	/******************Fore testing rowsums**********************
    	try {

    		File rowSumsFile = new File("C:/Users/sarita/Desktop/GSEA-TestData/rowSums_JAVA.txt");
    		Vector genesetNames=gSets[0].getAllGenesetNames();

    		PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(rowSumsFile)));
        	for(int k=0; k<genesetNames.size(); k++){
    			pw.write((String)genesetNames.get(k));
    			pw.write('\t');
    			pw.write(((Float)rowSums.get(k)).toString());
    			pw.write('\n');
    		}
        		pw.close();
    		} catch (IOException e) {
    	...
    	//System.out.println("Printing the AssoctionMatrix, after calculating rowSums");

    	/*	try {

		File AMatFile = new File("C:/Users/sarita/Desktop/GSEA-TestData/AMAT.txt");
		Vector _temp1=gSets[0].getAllGenesetNames();
		Vector _temp2=new Vector();
		int count=0;
		for(int k=0; k<_temp1.size(); k++){
			if(!_tempgeneset.contains((String)_temp1.get(k))){
					_temp2.add(count, _temp1.get(k));
					count=count+1;
			}
		}
		...
		PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(AMatFile)));
    	for(int k=0; k<unique_genes_in_dataset.size(); k++){
			pw.write((String)unique_genes_in_dataset.get(k));
			pw.write('\t');
		}
    	pw.write('\n');

    	for(int i=0; i<aMatrix.getRowDimension(); i++){
    		pw.write((String)_temp2.get(i));
    		pw.write('\t');
    		for(int j=0; j<aMatrix.getColumnDimension(); j++){

    			pw.write(((Float)aMatrix.get(i, j)).toString());
    			pw.write('\t');
    		}
    		pw.write('\n');
    	}
