...
public class GSEADataPanel extends JPanel implements IWizardParameterPanel {
	...
	/**
	 * Creates new form DataPanel IData would be required here, in order to
	 * initialize the GUI. To check if annotations are loaded or not we need a
	 * reference to IData.
	 * 
	...
	public GSEADataPanel(IData idata, AlgorithmData algData, JFrame parent,
	...
	private void initComponents() {
		...
		groupAssignment.setBorder(BorderFactory
	...
	/*	downloadGenesetLabel = new javax.swing.JLabel();
		...
		browseButton2.setBorder(BorderFactory
				.createBevelBorder(BevelBorder.RAISED));
		...
		groupAssignmentPanel.setBorder(new TitledBorder(new EtchedBorder(),
				"Phenotype/Class Assignment"));

		factorLabel.setText("Assign phenotype/Class labels to your samples");
		groupAssignment.setText("Assign");

		gba.add(groupAssignmentPanel, factorLabel, 0, 0, 1, 1, 0, 0, GBA.H,
				GBA.C, new Insets(5, 5, 5, 5), 0, 0);
		gba.add(groupAssignmentPanel, emptyLabel, 1, 0, 0, 0, 1, 0, GBA.H,
				GBA.C, new Insets(5, 5, 5, 5), 0, 0);
		gba.add(groupAssignmentPanel, groupAssignment, 2, 0, GBA.RELATIVE, 1,
				0, 0, GBA.NONE, GBA.C, new Insets(5, 5, 10, 5), 0, 0);
		gba.add(groupAssignmentPanel, assignmentSuccessfulLabel, 0, 1, 2, 1, 0,
				0, GBA.H, GBA.C, new Insets(5, 5, 5, 5), 0, 0);
	...
	/*	gba.add(genesetPanel, downloadGenesetLabel, 0, 1, 2, 1, 0, 0, GBA.H,
		...
		gba.add(genesetPanel, uploadGeneSetLabel, 0, 3, 1, 1, 0, 0, GBA.H,
				GBA.C, new Insets(5, 5, 5, 5), 0, 0);
		gba.add(genesetPanel, geneSetTextField, 1, 3, 1, 0, 1, 0, GBA.H, GBA.C,
				new Insets(5, 5, 5, 5), 0, 0);
		gba.add(genesetPanel, browseButton2, 2, 3, GBA.RELATIVE, 1, 0, 0,
				GBA.NONE, GBA.C, new Insets(5, 5, 5, 5), 0, 0);
		...
		gba.add(fileLoaderPanel, groupAssignmentPanel, 0, 0, 1, 1, 1, 1, GBA.B,
				GBA.C, new Insets(2, 2, 2, 2), 0, 0);
		gba.add(fileLoaderPanel, genesetPanel, 0, 2, 1, 1, 1, 1, GBA.B, GBA.C,
				new Insets(2, 2, 2, 2), 0, 0);
		gba.add(fileLoaderPanel, annotationPanel, 0, 3, 1, 1, 1, 1, GBA.B,
				GBA.C, new Insets(2, 2, 2, 2), 0, 0);
	...
	/**
	 * Initializes data panel based on params. If annotation has already been
	 * loaded, disable the browse and connect button, also show the path of the
...
//			adh.onClickAnnDownload();
	...
	public void getGroupAssignment() {
		...
		if (gBox1.isOkPressed()) {
			...
			gBox2 = new GSEAInitBox2(new JFrame(), true, Integer.parseInt(gBox1
					.getNumberofFactors()));
		...
		if (gBox2.isOkPressed()) {
			...
			gBox3 = new GSEAInitBox3(parentFrame, true, Integer.parseInt(gBox1
					.getNumberofFactors()), exptNamesVector, factorNames,
					numFactorLevels, this.clusterRepository);
			...
			if (gBox3.isOkPressed()) {
				if (Integer.parseInt(gBox1.getNumberofFactors()) == 3) {
					...
					assignments[0] = gBox3.getFactorAAssignments();
				this.assignmentSuccessfulLabel
						.setText("You have successfully assigned phenotypes.");
	...
	public void onGeneSetDownload() {
		...
		try {
			...
			Hashtable<ISupportFileDefinition, File> supportfilesHash = framework
					.getMultipleSupportFiles(mdef);
			Enumeration<ISupportFileDefinition> supportfiles = supportfilesHash
					.keys();

			if (supportfilesHash.size() > 0) {
				File genesetFile = supportfilesHash.get(supportfiles
						.nextElement());
				// System.out.println("Gene set file path
				// is:"+genesetFile.getAbsolutePath());
				geneSetTextField.setText(genesetFile.getAbsolutePath());
				downloadStatusLabel
						.setText("Gene set file download was successful");
				downloadStatusLabel.setForeground(Color.RED);
