 ...
 * @version 
 */
public class DEGseqInitBox extends AlgorithmDialog {
    ...
    public DEGseqInitBox(
        ...
        buildConstraints(constraints, 0, 0, 1, 1, 1, 20);
        paramsPane.add( methodsPanel, constraints);
        
        pPanel = new PValuePanel();
        buildConstraints(constraints, 0, 1, 1, 1, 1, 20);
        paramsPane.add(pPanel, constraints);
		
		JTabbedPane consolidatedPane = new JTabbedPane(); 
		//consolidatedPane.setLayout(gridbag);
		//gridbag.setConstraints(paramsPane, constraints);
		consolidatedPane.add("Parameters", paramsPane);
		 
      
        buildConstraints(constraints, 0, 1, 1, 1, 1, 20);
        gridbag.setConstraints(consolidatedPane, constraints);
        pane.add(consolidatedPane,constraints);   
	 ...
	 */
    class MethodsPanel extends JPanel implements ActionListener {
    	
    	JRadioButton buttonLRT, buttonFET, buttonMARS, buttonMATR;
    	JLabel warningMATR;
    	...
    	public MethodsPanel(){
            ...
            this.setLayout(gridbag);
            buttonLRT = new JRadioButton("LRT: Likelihood Ratio Test (Marioni et al. 2008)", false);
            buttonLRT.setActionCommand("");
            buttonFET = new JRadioButton("FET: Fisher's Exact Test (Joshua et al. 2009)", false);
            buttonFET.setActionCommand("");
            buttonMARS = new JRadioButton("MARS: MA-plot-based method with Random Sampling model (Wang et al. 2009)", false);
            buttonMARS.setActionCommand("");
            buttonMATR = new JRadioButton("MATR: MA-plot-based method with Technical Replicates (Wang et al. 2009)", false);
            buttonMATR.setActionCommand(MATR_Cmd);
            
            ButtonGroup buttonGroup = new ButtonGroup();
            buttonGroup.add(buttonLRT);
            buttonGroup.add(buttonFET);
            buttonGroup.add(buttonMARS);
            buttonGroup.add(buttonMATR);
            buttonMARS.setSelected(true);
            
            buttonLRT.addActionListener(this);
            buttonFET.addActionListener(this);
            buttonMARS.addActionListener(this);
            buttonMATR.addActionListener(this);
            
            buildConstraints(constraints, 0, 0, 1, 1, 50, 50);
            constraints.anchor = GridBagConstraints.LINE_START;
            gridbag.setConstraints(buttonLRT, constraints);
            this.add(buttonLRT);  

            buildConstraints(constraints, 0, 1, 1, 1, 50, 50);
            constraints.anchor = GridBagConstraints.LINE_START;
            gridbag.setConstraints(buttonFET, constraints);
            this.add(buttonFET);
            
            buildConstraints(constraints, 0, 2, 1, 1, 50, 50);
            constraints.anchor = GridBagConstraints.LINE_START;
            gridbag.setConstraints(buttonMARS, constraints);
            this.add(buttonMARS);
            
            buildConstraints(constraints, 0, 3, 1, 1, 50, 50);
            constraints.anchor = GridBagConstraints.LINE_START;
            gridbag.setConstraints(buttonMATR, constraints);
            this.add(buttonMATR);
            
            warningMATR = new JLabel("");
            warningMATR.setForeground(Color.red);
            buildConstraints(constraints, 0, 4, 1, 1, 10, 10);
            constraints.anchor = GridBagConstraints.CENTER;
            gridbag.setConstraints(warningMATR, constraints);
            this.add(warningMATR);
            
            this.validate();
    	}
    	
		public void actionPerformed(ActionEvent e) {
			warningMATR.setText(e.getActionCommand());
		}
    	
    }
        
    public int getMethod(){
		if (methodsPanel.buttonLRT.isSelected())
			return 1;
		if (methodsPanel.buttonFET.isSelected())
			return 2;
		if (methodsPanel.buttonMARS.isSelected())
			return 3;
		if (methodsPanel.buttonMATR.isSelected())
			return 4;
		return 0;
	}
    ...
    class PValuePanel extends JPanel {
        ...
        public PValuePanel() {
	        ...
	        //buildConstraints(constraints, 0, 1, 2, 1, 10, 25);
	        //constraints.anchor = GridBagConstraints.EAST;
    ...
    public class EventListener extends WindowAdapter implements ActionListener{
        ...
        public void actionPerformed(ActionEvent ae) {
            ...
            if(command.equals("ok-command")){
                ...
                HelpWindow hw = new HelpWindow(DEGseqInitBox.this, "DEGseq Initialization Dialog");
                okPressed = false;
    ...
    public int getTestDesign() {
        ...
        //if (mPanel.chooseDesignPane.getSelectedIndex() == 0) {
        	//design = DEGseqInitBox.ONE_CLASS;
        //} else if (mPanel.chooseDesignPane.getSelectedIndex() == 1) {
        	design = DEGseqInitBox.TWO_CLASS;
