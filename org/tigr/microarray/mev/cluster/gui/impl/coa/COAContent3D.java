 ...
 * COAContent3D.java
 *
 * Created on September 21, 2004, 1:04 PM
 */
...
public class COAContent3D extends JPanel {
    ...
    private FloatMatrix U, geneUMatrix, exptUMatrix, scaledGeneUMatrix, scaledExptUMatrix;
    ...
    /** Creates a new instance of COAContent3D */
    public COAContent3D(FloatMatrix U, Experiment experiment, int geneOrExpt) {
    ...
    public COAContent3D(FloatMatrix U, Experiment experiment, int geneOrExpt, int xAxis, int yAxis, int zAxis) {
        this.geneOrExpt = geneOrExpt;
        this.xAxis = xAxis;
        this.yAxis = yAxis;
        this.zAxis = zAxis;
    ...
    /*
    public COAContent3D(FloatMatrix geneUMatrix, FloatMatrix exptUMatrix, Experiment experiment, int geneOrExpt) {
        this.geneOrExpt = geneOrExpt;
        this.geneUMatrix = geneUMatrix;
        this.exptUMatrix = exptUMatrix;
        this.experiment = experiment;
        scaleMatrices(geneUMatrix, exptUMatrix);
        combineMatrices(scaledGeneUMatrix, scaledExptUMatrix);
        initScales(scaledGeneUMatrix, scaledExptUMatrix);
        setLayout(new BorderLayout());
        setPreferredSize(new Dimension(10, 10));
        GraphicsConfiguration config = SimpleUniverse.getPreferredConfiguration();
        this.onScreenCanvas = new Canvas3D(config);
    ...
    public COAContent3D(FloatMatrix geneUMatrix, FloatMatrix exptUMatrix, Experiment experiment, int geneOrExpt) {
        ...
        this.U = combineMatrices(scaledGeneUMatrix, scaledExptUMatrix);
        this.geneOrExpt = geneOrExpt;
        //this.U = U;
        this.experiment = experiment;
        initScales(U);
    ...
    public COAContent3D(FloatMatrix geneUMatrix, FloatMatrix exptUMatrix, Experiment experiment, int geneOrExpt, int xAxis, int yAxis, int zAxis) {
        this.geneUMatrix = geneUMatrix;
        this.exptUMatrix = exptUMatrix;        
        //scaleMatrices(geneUMatrix, exptUMatrix);
        scaledGeneUMatrix = (FloatMatrix)(geneUMatrix.clone());
    ...
    public void setShowGeneTextFromBoth(boolean value) {
        showGeneTextFromBoth = value;
    }    
    
    public void setShowExptTextFromBoth(boolean value) {
        showExptTextFromBoth = value;
    }    
    ...
    public boolean isShowGeneTextFromBoth() {
        return showGeneTextFromBoth;
    }   
    
    public boolean isShowExptTextFromBoth() {
        return showExptTextFromBoth;
    }    
...
//        ImageComponent2D buffer = new ImageComponent2D(ImageComponent.FORMAT_RGB, (BufferedImage)this.createImage(onScreenCanvas.getWidth(), onScreenCanvas.getHeight()));
    ...
    private void scaleMatrices(FloatMatrix U1, FloatMatrix U2) { //brings them both to the same scale for plotting
        ...
        for (int i = rows1; --i >= 0;) {
            ...
            max1 = Math.max(max1, Math.max(Math.max(Math.abs(U1.get(i, xAxis)), Math.abs(U1.get(i, yAxis))), Math.abs(U1.get(i, zAxis))));
        }
        for (int i = rows2; --i >= 0;) {
    ...
    private void initScales(FloatMatrix U1, FloatMatrix U2) {
        float max1 = 0f;
        float max2 = 0f;
        final int rows1 = U1.getRowDimension();
        final int rows2 = U2.getRowDimension();
        ...
        for (int i = rows2; --i >= 0;) {
            //max2 = Math.max(max2, Math.max(Math.max(Math.abs(U2.get(i, 0)), Math.abs(U2.get(i, 1))), Math.abs(U2.get(i, 2))));
            max2 = Math.max(max2, Math.max(Math.max(Math.abs(U2.get(i, xAxis)), Math.abs(U2.get(i, yAxis))), Math.abs(U2.get(i, zAxis))));
        }  
    ...
    private TransformGroup createCoordinateSystem(BoundingSphere bounds) {
        ...
        if (isShowSpheres()) {
            ...
            if (getPointSize() > 0) {
                ...
                if ((geneOrExpt == COAGUI.GENES) || (geneOrExpt == COAGUI.EXPTS)) {
                    ...
                    Shape3D[] points = createSelectedPointsFromBoth();
                    if (points != null) {
                        for (int i = 0; i < points.length; i++) {
                            group.addChild(points[i]);
                        }
                    }
                }
            }
        ...
        if (geneOrExpt == COAGUI.BOTH) {
            if (isShowGeneTextFromBoth()) {
                group.addChild(createGeneTextFromBoth());
            }  
            if (isShowExptTextFromBoth()) {
                group.addChild(createSampleTextFromBoth());
            }            
        } else {
            if (isShowText()) {
    ...
    protected int[] getSelectedGenesFromBoth() {
        Vector selectedPointsVector = new Vector();
        for (int i = 0; i < geneUMatrix.getRowDimension(); i++) {
            //if (isPointSelected(U.get(i,0), U.get(i,1), U.get(i,2))) {
            if (isPointSelected(U.get(i,xAxis), U.get(i,yAxis), U.get(i,zAxis))) {
                selectedPointsVector.add(new Integer(experiment.getGeneIndexMappedToData(i)));
            }
        }
        
        int[] selPoints = new int[selectedPointsVector.size()];
        for (int i = 0; i < selectedPointsVector.size(); i++) {
            selPoints[i] = ((Integer)(selectedPointsVector.get(i))).intValue();
        }
        return selPoints;
    }
    ...
    protected int[] getSelectedExptsFromBoth() {
        Vector selectedPointsVector = new Vector();
        for (int i = geneUMatrix.getRowDimension(); i < U.getRowDimension(); i++) {
            //if (isPointSelected(U.get(i,0), U.get(i,1), U.get(i,2))) {
            if (isPointSelected(U.get(i,xAxis), U.get(i,yAxis), U.get(i,zAxis))) {
                selectedPointsVector.add(new Integer(experiment.getSampleIndex(i - geneUMatrix.getRowDimension())));
            }
        }
    ...
    private Shape3D[] createSelectedPointsFromBoth() {
        ...
        Shape3D[] selectedPoints = new Shape3D[2];
        boolean drawGenes = true;
        if (drawGenes) {
            Color3f color = new Color3f(1.0f, 0.3f, 1.0f);
            Material material = new Material(color, color, color, color, 100.0f);
            material.setLightingEnable(true);
        ...
        if (!drawGenes) {
            ...
            material.setLightingEnable(true);
            Appearance appearance = new Appearance();
            float exptSelectedPointSize = getSelectedPointSize();
            if (exptSelectedPointSize < 5.0f) exptSelectedPointSize = 5.0f;
            appearance.setPointAttributes(new PointAttributes(exptSelectedPointSize, false));
            appearance.setMaterial(material);
    ...
    /**
     * Creates a point appearance with specified colorand point size.
     */
    private Appearance createPointAppearance(Color3f color, float pSize) {
        Material material = new Material(color, color, color, color, 100.0f);
        material.setLightingEnable(true);
        Appearance appearance = new Appearance();
        appearance.setPointAttributes(new PointAttributes(pSize, false));
        appearance.setMaterial(material);
        return appearance;
    }
...
private Shape3D[] createUsualPoints() {
    Shape3D[] pointShapes = new Shape3D[1];
    Shape3D[] genePointShapes = new Shape3D[1];
    Shape3D[] exptPointShapes = new Shape3D[1];
    ...
    if ((geneOrExpt == COAGUI.GENES) || (geneOrExpt == COAGUI.EXPTS)) {
        ...
        } else if (geneOrExpt == COAGUI.EXPTS){ //Experiment Viewer
            for (int i=0; i<colors.length; i++) {
                color = new Color3f(colors[i]);
                appearances[i+delta] = createPointAppearance(color);
        ...
        pointShapes = new Shape3D[pointArrays.length];
        for (int i=0; i<pointShapes.length; i++) {
        ...
        boolean createGenePoints = true;
        
        if (createGenePoints) {
            int geneUncoloredCount = data.getColoredProbesCount(-1);
            int geneDelta = geneUncoloredCount == 0 ? 0 : 1;
            
            Color[] geneColors = data.getColors();
            
            PointArray[] genePointArrays = new PointArray[geneColors.length+geneDelta];
            Appearance[] geneAppearances = new Appearance[geneColors.length+geneDelta];
            int[] geneCounters = new int[geneColors.length+geneDelta];
            Color3f geneColor;
            
            if (geneUncoloredCount > 0) {
                geneAppearances[0] = createPointAppearance(isWhiteBackground() ? blackColor : whiteColor);
                genePointArrays[0] = new PointArray(geneUncoloredCount, PointArray.COORDINATES | PointArray.COLOR_3 | PointArray.NORMALS);
                geneCounters[0]    = 0;
            }
            
            for (int i=0; i<geneColors.length; i++) {
                geneColor = new Color3f(geneColors[i]);
                geneAppearances[i+geneDelta] = createPointAppearance(geneColor);                 
                genePointArrays[i+geneDelta] = new PointArray(data.getColoredProbesCount(i), PointArray.COORDINATES | PointArray.COLOR_3 | PointArray.NORMALS);
                geneCounters[i+geneDelta]    = 0;
            }   
            ...
            for (int i=0; i<scaledGeneUMatrix.getRowDimension(); i++) {
                //x = scaledGeneUMatrix.get(i,0);
                //y = scaledGeneUMatrix.get(i,1);
                //z = scaledGeneUMatrix.get(i,2);
                x = scaledGeneUMatrix.get(i,xAxis);
                y = scaledGeneUMatrix.get(i,yAxis);
                z = scaledGeneUMatrix.get(i,zAxis);                
                if (!isSelection() || !isPointSelected(x, y, z)) {
                    //if(geneOrExpt == COAGUI.GENES)
                    index = data.getProbeColorIndex(experiment.getGeneIndexMappedToData(i))+geneDelta;
                    //else if (geneOrExpt == COAGUI.EXPTS)
                    //index = data.getExperimentColorIndex(i)+delta;
                    
                    genePointArrays[index].setCoordinate(geneCounters[index], new Point3f(x*factorX, y*factorY, z*factorZ));
                    geneCounters[index]++;
                }
            }     
            
            genePointShapes = new Shape3D[genePointArrays.length];
            for (int i=0; i<genePointShapes.length; i++) {
                genePointShapes[i] = new Shape3D();
                genePointShapes[i].setAppearance(geneAppearances[i]);
                genePointShapes[i].setGeometry(genePointArrays[i]);
            }

        } // end if (createGenePoints)
        
        createGenePoints = false;
        
        if (!createGenePoints) { // i.e., create expt points
            int exptUncoloredCount = data.getColoredExperimentsCount(-1);
            int exptDelta = exptUncoloredCount == 0 ? 0 : 1;            
            
            Color[] exptColors = data.getExperimentColors();
            
            PointArray[] exptPointArrays = new PointArray[exptColors.length+ exptDelta];
            Appearance[] exptAppearances = new Appearance[exptColors.length+ exptDelta];
            int[] exptCounters = new int[exptColors.length+ exptDelta];
            Color3f exptColor;  
            
            // not published probes
            if (exptUncoloredCount > 0) {
                if (getPointSize() < 10.0f) {
                    exptAppearances[0] = createPointAppearance(isWhiteBackground() ? blackColor : whiteColor, 10.0f);
                } else {
                    exptAppearances[0] = createPointAppearance(isWhiteBackground() ? blackColor : whiteColor);
                }
                exptPointArrays[0] = new PointArray(exptUncoloredCount, PointArray.COORDINATES | PointArray.COLOR_3 | PointArray.NORMALS);
                exptCounters[0]    = 0;
            }    
            
            for (int i=0; i<exptColors.length; i++) {
                exptColor = new Color3f(exptColors[i]);
                if (getPointSize() < 10.0f) {
                    exptAppearances[i+exptDelta] = createPointAppearance(exptColor, 10.0f);
                } else {
                    exptAppearances[i+exptDelta] = createPointAppearance(exptColor);
                }
                exptPointArrays[i+exptDelta] = new PointArray(data.getColoredExperimentsCount(i), PointArray.COORDINATES | PointArray.COLOR_3 | PointArray.NORMALS);
                exptCounters[i+exptDelta]    = 0;
            }         
            ...
            for (int i=0; i<scaledExptUMatrix.getRowDimension(); i++) {
                //x = scaledExptUMatrix.get(i,0);
                //y = scaledExptUMatrix.get(i,1);
                //z = scaledExptUMatrix.get(i,2);
                x = scaledExptUMatrix.get(i,xAxis);
                y = scaledExptUMatrix.get(i,yAxis);
                z = scaledExptUMatrix.get(i,zAxis);                
                if (!isSelection() || !isPointSelected(x, y, z)) {
                    //if(geneOrExpt == COAGUI.GENES)
                    //index = data.getProbeColorIndex(experiment.getGeneIndexMappedToData(i))+delta;
                    // else if (geneOrExpt == COAGUI.EXPTS)
                    index = data.getExperimentColorIndex(i)+exptDelta;
                    
                    exptPointArrays[index].setCoordinate(exptCounters[index], new Point3f(x*factorX, y*factorY, z*factorZ));
                    exptCounters[index]++;
                }
            }     
            
            exptPointShapes = new Shape3D[exptPointArrays.length];
            for (int i=0; i<exptPointShapes.length; i++) {
                exptPointShapes[i] = new Shape3D();
                exptPointShapes[i].setAppearance(exptAppearances[i]);
                exptPointShapes[i].setGeometry(exptPointArrays[i]);
            }          

        } // end if (!createGenePoints)
        
        pointShapes = new Shape3D[genePointShapes.length + exptPointShapes.length];
        for (int i = 0; i < genePointShapes.length; i++) {
            pointShapes[i] = genePointShapes[i];
        }
        for (int i = 0; i < exptPointShapes.length; i++) {
            pointShapes[genePointShapes.length + i] = exptPointShapes[i];
        }
        
    } // end if (geneOrExpt == COAGUI.BOTH)
    ...
    private Shape3D[] createUsualPointsOld() {
        ...
        if (geneOrExpt == COAGUI.GENES) {
            ...
            uncoloredCount = data.getColoredProbesCount(-1) + data.getColoredExperimentsCount(-1);
        }
        System.out.println("unColoredCount = " + uncoloredCount);
        int delta = uncoloredCount == 0 ? 0 : 1;
        int geneDelta = data.getColoredProbesCount(-1) == 0 ? 0 : 1;
        int exptDelta = data.getColoredExperimentsCount(-1) == 0 ? 0 : 1;
        //System.out.println("delta = " + delta);
        
        Color[] colors = new Color[1];
        Color[] geneColors = new Color[1];
        Color[] exptColors = new Color[1];
        int[] bothColorCounts = new int[1];
        if(geneOrExpt == COAGUI.GENES)
            colors = data.getColors(); // get colored gene clusters
        else if (geneOrExpt == COAGUI.EXPTS)
            colors = data.getExperimentColors();
        else if (geneOrExpt == COAGUI.BOTH) {
            geneColors = data.getColors();
            exptColors = data.getExperimentColors();
             
            System.out.println("geneColors.length = " + geneColors.length + ", exptColors.length = " + exptColors.length + ", colors.length = " + colors.length + ", bothColorCounts.length = " + bothColorCounts.length);
            
            colors = new Color[geneColors.length + exptColors.length];
            for (int i = 0; i < geneColors.length; i++) {
                colors[i] = geneColors[i];
            }
            for (int i = 0; i < exptColors.length; i++) {
                colors[geneColors.length + i] = exptColors[i];
            }
        ...
        Appearance[] appearances = new Appearance[colors.length+delta];
        int[] counters = new int[colors.length+delta];
        int[] exptCounters = new int[colors.length + delta];
        
        if (geneOrExpt == COAGUI.BOTH) {
            pointArrays = new PointArray[colors.length+geneDelta + exptDelta];
            appearances = new Appearance[colors.length+ geneDelta + exptDelta];
            counters = new int[colors.length+ geneDelta + exptDelta];
            exptCounters = new int[colors.length + geneDelta + exptDelta];
        }
        ...
        // not published probes
            ...
            if (data.getColoredProbesCount(-1) > 0) {
                appearances[0] = createPointAppearance(isWhiteBackground() ? blackColor : whiteColor);
                pointArrays[0] = new PointArray(data.getColoredProbesCount(-1), PointArray.COORDINATES | PointArray.COLOR_3 | PointArray.NORMALS);
                counters[0]    = 0;
                exptCounters[0] = 0;                
            }
            if (data.getColoredExperimentsCount(-1) > 0) {
                appearances[geneColors.length + geneDelta] = createPointAppearance(isWhiteBackground() ? blackColor : whiteColor);
                pointArrays[geneColors.length + geneDelta] = new PointArray(data.getColoredExperimentsCount(-1), PointArray.COORDINATES | PointArray.COLOR_3 | PointArray.NORMALS);
                counters[geneColors.length + geneDelta]    = 0;
                exptCounters[geneColors.length + geneDelta] = 0;                  
            }
        }
        if(geneOrExpt == COAGUI.GENES){
            // published ones
            ...
            for (int i=0; i<geneColors.length; i++) {
                color = new Color3f(colors[i]);
                appearances[i+geneDelta] = createPointAppearance(color);
                pointArrays[i+geneDelta] = new PointArray(data.getColoredProbesCount(i), PointArray.COORDINATES | PointArray.COLOR_3 | PointArray.NORMALS);
                counters[i+geneDelta]    = 0;
            }    
            for (int i=0; i<exptColors.length; i++) {
                color = new Color3f(colors[geneColors.length + i]);
                appearances[geneColors.length + geneDelta + i + exptDelta] = createPointAppearance(color);
                pointArrays[geneColors.length + geneDelta + i + exptDelta] = new PointArray(data.getColoredExperimentsCount(i), PointArray.COORDINATES | PointArray.COLOR_3 | PointArray.NORMALS);
                counters[geneColors.length + geneDelta + i + exptDelta]    = 0;
                exptCounters[geneColors.length + geneDelta + i + exptDelta] = 0;
                //exptCounters[geneColors.length + geneDelta + i + exptDelta] = 0;
            }           
        }
        ...
        System.out.println("U.getRowDimension() = " + U.getRowDimension());
        ...
        for (int i=0; i<U.getRowDimension(); i++) {
            ...
            if (!isSelection() || !isPointSelected(x, y, z)) {
                if(geneOrExpt == COAGUI.GENES) {
                    index = data.getProbeColorIndex(experiment.getGeneIndexMappedToData(i))+delta;
                    System.out.println("i = " + i+ ", index = " + index + ", counters[index] = " + counters[index]);
                    pointArrays[index].setCoordinate(counters[index], new Point3f(x*factorX, y*factorY, z*factorZ));
                    counters[index]++;
                }
                else if (geneOrExpt == COAGUI.EXPTS) {
                    ...
                    if (i < geneUMatrix.getRowDimension()) {
                       index = data.getProbeColorIndex(experiment.getGeneIndexMappedToData(i))+ geneDelta;
                       System.out.println("i < geneUMatrix.getRowDimension(): i = " + i+ ", index = " + index + ", counters[index] = " + counters[index]);
                       pointArrays[index].setCoordinate(counters[index], new Point3f(x*factorX, y*factorY, z*factorZ));
                       counters[index]++;
                    }
                    else {
                        //index = geneColors.length + data.getExperimentColorIndex(i - geneUMatrix.getRowDimension())+delta;
                        //System.out.println("data.getExperimentColorIndex(i - geneUMatrix.getRowDimension()) = " + data.getExperimentColorIndex(i - geneUMatrix.getRowDimension()));
                        //System.out.println("geneDelta = " + geneDelta + ", exptDelta = " + exptDelta);
                        int initExptIndex = data.getExperimentColorIndex(i - geneUMatrix.getRowDimension()) + exptDelta;
                        if (initExptIndex == 0){
                            index = 0;
                        } else {
                            index = data.getExperimentColorIndex(i - geneUMatrix.getRowDimension()) + exptDelta + (data.getColors()).length;
                        }
                        System.out.println("i >= geneUMatrix.getRowDimension(): i = " + i+ ", index = " + index + ", exptCounters[index] = " + exptCounters[index]);
                        pointArrays[index].setCoordinate(exptCounters[index], new Point3f(x*factorX, y*factorY, z*factorZ));
                        exptCounters[index]++;                     
                        //pointArrays[geneColors.length+ index].setCoordinate(counters[geneColors.length + index], new Point3f(x*factorX, y*factorY, z*factorZ));
                        //counters[geneColors.length + index]++;
                    }
                }
        ...
        for (int i=0; i<U.getRowDimension(); i++) {
            ...
            if (geneOrExpt == COAGUI.GENES){
                ...
                if (i < geneUMatrix.getRowDimension()) {
                    ...
                    if (data.getExperimentColor(i - geneUMatrix.getRowDimension()) == null) {
                        sphere.addChild(new Sphere((float)(2f*getPointSize(selected)/20f), (selected ? sAppearance : uAppearance)));
                    } else {
                        sphere.addChild(new Sphere((float)(2f*getPointSize(selected)/20f), (selected ? sAppearance : createSphereAppearance(new Color3f(data.getExperimentColor(i - geneUMatrix.getRowDimension()))))));
                    }                    
                }
            }
    ...
    private TransformGroup createText() {
        ...
        for (int i=0; i<U.getRowDimension(); i++) {
            ...
            text = "";
            if (geneOrExpt == COAGUI.GENES) {
                //text = data.getUniqueId(i)
                text = data.getElementAttribute(experiment.getGeneIndexMappedToData(i), geneLabelIndex);
            } else if (geneOrExpt == COAGUI.EXPTS) {
                text = data.getSampleName(experiment.getSampleIndex(i));
            } else if (geneOrExpt == COAGUI.BOTH) {
                if (i < geneUMatrix.getRowDimension()) {
                    text = data.getElementAttribute(experiment.getGeneIndexMappedToData(i), geneLabelIndex);
                } else {
                    text = data.getSampleName(experiment.getSampleIndex(i - geneUMatrix.getRowDimension()));
                }
            }
