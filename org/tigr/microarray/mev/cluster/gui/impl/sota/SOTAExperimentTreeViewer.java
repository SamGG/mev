...
public class SOTAExperimentTreeViewer extends JPanel implements IViewer{
    ...
    private void readObject(java.io.ObjectInputStream ois) throws java.io.IOException, ClassNotFoundException {
        ...
        this.expViewer = (SOTAExpCentroidExpressionViewer)ois.readObject();
 ...
 */
            else if(actionCmd.equals(SOTAExperimentTreeViewer.this.SET_CLUSTER_CMD)){
                if(currClusterNum != -1)
                    onSetCluster(currClusterNum);
            }
            else if(actionCmd.equals(SOTAExperimentTreeViewer.this.SAVE_CLUSTER_CMD)){
                if(currClusterNum != -1)
                    onSaveCluster(currClusterNum);
            }
            else if(actionCmd.equals(SOTAExperimentTreeViewer.this.DELETE_CLUSTER_CMD)){
                if(currClusterNum != -1)
                    onDeleteCluster(currClusterNum);
            }
            
            else if(actionCmd.equals(SOTAExperimentTreeViewer.this.DELETE_ALL_CLUSTERS_CMD)){
                onDeleteAllClusters();
            }
        }
