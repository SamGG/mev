...
//		gseaInitWizard.setParamSufficient(validateSampleSelection());
	...
	public void makeClusterSelector(){
		...
		for (int i=0; i<getNumberOfFactors(); i++){
			...
			clusterSelector[i].addMouseMotionListener(new MouseMotionListener(){
	...
	public boolean validateSampleSelection() {
		for (int i=0; i<this.num_factors; i++){
			int[] groupAssign;
			if (!((MultiGroupExperimentsPanel) gPanel.getComponent(i)).isButtonSelectionMethod())
				groupAssign =  getFactorClusterAssignments(i);
			else
				groupAssign = ((MultiGroupExperimentsPanel)gPanel.getComponent(i)).getGroupAssignments();
