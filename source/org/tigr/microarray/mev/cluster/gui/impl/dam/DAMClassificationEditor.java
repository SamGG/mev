...
//   				int[] groupAssgn = getGroupAssignments();
	        ...
	        int numRows = damTabModel.getRowCount(); 
			for(int sample = 0; sample < numRows; sample++) {
				pw.print(String.valueOf(sample+1)+"\t"); //sample index
				pw.print(damTabModel.getValueAt(sample, numberOfClasses + 2)+"\t");
				for (int j = 1; j <= numberOfClasses; j++) {
                    if (((Boolean)(damTabModel.getValueAt(sample, j))).booleanValue()) {
                        pw.println("Class "+j);
                        break;
                    }
                }
                if (((Boolean)(damTabModel.getValueAt(sample, numberOfClasses + 1))).booleanValue()) {
                    pw.println("Exclude");
                }
			}
			pw.flush();
    ...
    private void loadFromFile(File file){
		...
		try {						
			...
			if (numberOfClasses!= groupNames.size()){
				Object[] optionst = { "Continue", "Cancel" };
				if (JOptionPane.showOptionDialog(null, 
					"The saved file was saved using "+groupNames.size()+" classes.\nWould you like MeV to attempt to load selections into " +numberOfClasses + " classes?", 
			...
			for(int sample = 0; sample < numRows; sample++) {
				...
				for (int i=0;i<numRows; i++){
					if (i==sample)
						continue;
					if (((String)damTabModel.getValueAt(sample, numberOfClasses + 2)).equals(
							((String)damTabModel.getValueAt(i, numberOfClasses + 2)))){
						doIndex=true;
					}
				}
				try{
					fileSampleIndex = sampleNames.indexOf((String)damTabModel.getValueAt(sample, numberOfClasses+2));
				}catch (Exception e){
					e.printStackTrace();
					fileSampleIndex=-1;
				}
				...
				if (groupIndex==-1||groupIndex>numberOfClasses)
					groupIndex = numberOfClasses;
				//set state
				
				try{
    				damTabModel.setValueAt(new Boolean(true), sample, groupIndex+1);
				}catch (Exception e){
					e.printStackTrace();
					damTabModel.setValueAt(new Boolean(true), sample, numberOfClasses+1);  //set to last state... excluded
				}
			}		
	...
	private void setStateBasedOnIndex(Vector<String>groupAssignments,Vector<String>groupNames){
		...
		for(int sample = 0; sample < numRows; sample++) {
			...
			//set state
			try{
				damTabModel.setValueAt(new Boolean(true), sample, groupNames.indexOf(groupAssignments.get(sample)+1));
			}catch (Exception e){
				e.printStackTrace();
				damTabModel.setValueAt(new Boolean(true), sample, numberOfClasses+1);
			}
		}
	}
