...
public class HCLTree extends JPanel implements java.io.Serializable {
    ...
    public static final int xOrigin = 20;
    
    protected HCLTreeListener treeListener;
    
    protected int orientation = HORIZONTAL;
    protected int min_pixels = 10;
    protected int max_pixels = 50;
	...
	public HCLTree(HCLTreeData treeData, Integer orientation, ArrayList dotsp){
        ...
        this.terminalNodes = new boolean[this.treeData.height.length];  //terminalNodes is a boolean array that has
        															    //the size of the number of genes
  ...
  public HCLTreeData getTreeData(){return this.treeData;}    
            ...
            switch (this.orientation) {
                case HORIZONTAL: 
                    ...
                    for (int i = 0; i < this.dots.size(); i++) {
                    	TEASEInfoBox dot = (TEASEInfoBox) this.dots.get(i);
                    	dot.resetX(this.pHeights[treeData.node_order[treeData.node_order.length-2]] - 
                    			this.pHeights[treeData.node_list[i]] + xOrigin);
                    }
                    break;
                case VERTICAL:
    ...
    /**
     * Returns height of the tree nodes.
     */
    private float getMaxHeight(int[] nodeOrder, float[] height) {
        ...
        //System.out.println("min height of the tree nodes"+max);
        return max;
    }
    ...
    /**
     * Calculates tree node y positions.
     */
...
//        System.out.println("HCLTree 511, positions[]");
...
//        }
    ...
    private int[] getPixelHeights(int[] nodeOrder, float[] height) {
        ...
        for (int i=0; i<nodeOrder.length-1; i++) {
            ...
            pHeights[node] = Math.max(pHeights[child_1], pHeights[child_2]) + 
			Math.max(Math.min((int)Math.round(height[node]*scale), max_pixels), min_pixels);
        }
//        System.out.println("HCLTree 553");
//        for (int i = 0 ; i < pHeights.length; i++)
//        	System.out.print(pHeights[i]+ "  ");
        return pHeights;
    }
