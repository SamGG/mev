...
public class NMFHCLViewer extends JPanel implements IViewer {
    ...
    public NMFHCLViewer(Experiment experiment, int[] features, HCLTreeData genes_result, HCLTreeData samples_result) {
        ...
        if (samples_result != null && samples_result.node_order != null && experiment.getNumberOfSamples() > 1 && samples_result.node_order.length > 1) {
            ...
            sampleTree.setAbsoluteHeight(true);
...
//        this.genesOrder = createGenesOrder(experiment, features, genes_result);
    ...
    protected int getCommonWidth() {
        ...
        if(this.isExperimentCluster)
            ...
            width += ((NMFHCLExperimentViewer)this.expViewer).getWidth();
    ...
    protected void addMenuItems(JPopupMenu menu, Listener listener) {
     ...
     /*   if(this.genesTree != null) {
       ...
       */
        ...
        menuItem.setActionCommand(NMFHCLExperimentViewer.BROADCAST_MATRIX_GAGGLE_CMD);
        menuItem.addActionListener(listener);
        menu.add(menuItem);
        
        menuItem = new JMenuItem("Broadcast Gene List to Gaggle", GUIFactory.getIcon("gaggle_icon_16.gif"));
        menuItem.setActionCommand(NMFHCLExperimentViewer.BROADCAST_NAMELIST_GAGGLE_CMD);
        menuItem.addActionListener(listener);
        menu.add(menuItem);
    }
    ...
    protected void setSelectedCluster(HCLCluster cluster) {
        ...
        if(this.isExperimentCluster){  //viewer type,
            ...
            if(cluster.isGeneCluster)
                ((NMFHCLExperimentViewer)this.expViewer).selectRows(cluster.firstElem, cluster.lastElem);
            else
                ((NMFHCLExperimentViewer)this.expViewer).selectColumns(cluster.firstElem, cluster.lastElem);
        }
    }
...
//            notANewNode = false;
    ...
    private void refreshViewer(){
        ...
        if(this.isExperimentCluster){
            ...
            ((NMFHCLExperimentViewer)(this.expViewer)).selectRows(-1,-1);
            ((NMFHCLExperimentViewer)(this.expViewer)).selectColumns(-1,-1);
        }
    }
...
//                onRotateNode();
        ...
        public void valueChanged(HCLTree source, HCLCluster cluster) {
            ...
            NMFHCLViewer.this.valueChanged(source, cluster);
        }
            ...
            if(source instanceof HCLTree){  //in a tree don't deselect
                if(source == genesTree) { //if colloring rows (genes)
                    ...
                    if(isExperimentCluster){
                        ...
                        ((NMFHCLExperimentViewer)expViewer).selectColumns(-1, -1);
                    }
                    if(sampleTree != null)
                    ...
                    if(isExperimentCluster){
                        ...
                        ((NMFHCLExperimentViewer)expViewer).selectRows(-1, -1);
                    }
                    if(genesTree != null)
