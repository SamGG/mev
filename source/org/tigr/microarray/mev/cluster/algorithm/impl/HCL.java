...
public class HCL extends AbstractAlgorithm {
	...
	private boolean nmf = false; // for use with NMF module, which uses no data
									// matrix and imports its own similarity
									// matrix
	...
	public AlgorithmData executeNMF(AlgorithmData data, float[][] simMat)
	...
	public AlgorithmData execute(AlgorithmData data) throws AlgorithmException {
		...
		function = map.getInt("hcl-distance-function", EUCLIDEAN);
		absolute = map.getBoolean("hcl-distance-absolute", false);
		genes = map.getBoolean("calculate-genes", true);
		optimizeOrdering = map.getBoolean("optimize-sample-ordering", false);
		if (genes)
			optimizeOrdering = map.getBoolean("optimize-gene-ordering", false);
		...
		// ======== Init =========
		int memoryAssess = ExperimentUtil.javaHCLMemoryAssess(n,
				optimizeOrdering);
		if (memoryAssess == 0)
			stop = false;
		if (memoryAssess == 1)
...
//		AlgorithmEvent event = null;
		...
		if (!nmf) {
			factor = (float) 1.0; // factor is used as an optional scaling
			ExecutorService pool = 	Executors.newFixedThreadPool(8);
			for (i = 1; i < n; ++i) {
				final int iFinal = i;
				Runnable runnable1 = new Runnable() {
					public void run() {
						fillSimMatRow(iFinal);
					}
				};
...
//		for (int q = 1; q < SimilarityMatrix.length; q++) {
//			System.out.print(SimilarityMatrix[q][0] + "\t");
//		}
		// ========================================
		...
		while (parentless > 1) { // main loop runs until every node except the
									// root node is assigned a parent node.
			if (stop) {
				throw new AbortException();
			}
			...
			CalculatedNodes++;
			double close_d = Double.POSITIVE_INFINITY; // first find the closest
														// pair
			...
			try {
				...
				if (owner[i] >= n && Height[owner[i]] > height_k) { // Gene1
																	// already
																	// assignd
																	// to a
																	// node, was
																	// >= ?!
					k = owner[i];
					AssertParentage(Parent, NumberOfChildren, Child1, Child2,
							owner[j], k);
				} else if (owner[j] >= n && Height[owner[j]] > height_k) { // Gene2
																			// already
																			// assignd
																			// to
																			// node
				...
				NodeHeight[k] = Math.max(NodeHeight[Child1[k]] + 1,
						NodeHeight[Child2[k]] + 1);
			} catch (Exception e) {
				e.printStackTrace();
				fireValueChanged(new AlgorithmEvent(this,
						AlgorithmEvent.WARNING, 0, "Error: " + e.toString()
								+ " - Height(" + String.valueOf(height_k) + ","
								+ ")"));
				k = 0;
			}
			...
			if (method == -1) { // minimum method
				for (p = 0; p < j; ++p)
					if (owner[p] != -1)
						SimilarityMatrix[i][p] = (float) Math.min(
								SimilarityMatrix[i][p], SimilarityMatrix[j][p]);
				for (p = j + 1; p < i; ++p)
					if (owner[p] != -1)
						SimilarityMatrix[i][p] = (float) Math.min(
								SimilarityMatrix[i][p], SimilarityMatrix[p][j]);
				for (p = i + 1; p < n; ++p)
					if (owner[p] != -1)
						SimilarityMatrix[p][i] = (float) Math.min(
								SimilarityMatrix[p][i], SimilarityMatrix[p][j]);
			} else if (method == +1) { // maximum method
				for (p = 0; p < j; ++p)
					if (owner[p] != -1)
						SimilarityMatrix[i][p] = (float) Math.max(
								SimilarityMatrix[i][p], SimilarityMatrix[j][p]);
				for (p = j + 1; p < i; ++p)
					if (owner[p] != -1)
						SimilarityMatrix[i][p] = (float) Math.max(
				...
				for (p = i + 1; p < n; ++p)
					if (owner[p] != -1)
						SimilarityMatrix[p][i] = (float) Math.max(
								SimilarityMatrix[p][i], SimilarityMatrix[p][j]);
			} else if (method == 2) { // average method
				// int
				// schrott=NumberOfChildren[owner[j]]+NumberOfChildren[owner[i]];
				// System.out.println(NumberOfChildren[owner[i]]);
				for (p = 0; p < j; ++p)
					if (owner[p] != -1)
						SimilarityMatrix[i][p] = (float) ((SimilarityMatrix[i][p]
								* NumberOfChildren[owner[i]] + SimilarityMatrix[j][p]
								* NumberOfChildren[owner[j]]) / (2.0 * Math
								.min(NumberOfChildren[owner[i]],
										NumberOfChildren[owner[j]])));
				for (p = j + 1; p < i; ++p)
					if (owner[p] != -1)
						SimilarityMatrix[i][p] = (float) ((SimilarityMatrix[i][p]
								* NumberOfChildren[owner[i]] + SimilarityMatrix[p][j]
								* NumberOfChildren[owner[j]]) / (2.0 * Math
								.min(NumberOfChildren[owner[i]],
										NumberOfChildren[owner[j]])));
				for (p = i + 1; p < n; ++p)
					if (owner[p] != -1)
						SimilarityMatrix[p][i] = (float) ((SimilarityMatrix[p][i]
								* NumberOfChildren[owner[i]] + SimilarityMatrix[p][j]
								* NumberOfChildren[owner[j]]) / (2.0 * Math
								.min(NumberOfChildren[owner[i]],
										NumberOfChildren[owner[j]])));
				...
				for (p = 0; p < j; ++p)
					if (owner[p] != -1)
						SimilarityMatrix[i][p] = (float) ((SimilarityMatrix[i][p] + SimilarityMatrix[j][p]) / 2.0);
				for (p = j + 1; p < i; ++p)
					if (owner[p] != -1)
						SimilarityMatrix[i][p] = (float) ((SimilarityMatrix[i][p] + SimilarityMatrix[p][j]) / 2.0);
				for (p = i + 1; p < n; ++p) {
					// System.out.println("p="+p+"\ti="+i+"\tn="+n);
//					if (owner == null || SimilarityMatrix[p] == null) {
//						System.out.println("p=" + p);
//						System.out.println(SimilarityMatrix[p]);
//					}
					try {
						if (owner[p] != -1)
							SimilarityMatrix[p][i] = (float) ((SimilarityMatrix[p][i] + SimilarityMatrix[p][j]) / 2.0);
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
		...
		if (optimizeOrdering && (n > 1)) {
			...
			MakeLeavesUnderMatrix(2 * n - 2, Child1, Child2, n, LeavesUnder,
					NumberOfChildren);
			...
			optevent = new AlgorithmEvent(this, AlgorithmEvent.SET_UNITS, n * n
					/ 2, "Optimizing Leaf Order");
			 ...
			 */
			for (int reset = 0; reset < n; reset++) { // Child arrays are
														// changed so that the
														// Child[i]=i for all
														// leaves
				Child1[reset] = reset;
			...
			MakeOptimalSumMatrix(Child1, Child2, NumberOfChildren, LeavesUnder,
					OptimalSum, SimilarityMatrix, n, (2 * n - 2), optevent);
			for (int reset = 0; reset < n; reset++) { // Child arrays are reset
														// to their original
														// values for leaves
				Child1[reset] = -1;
				Child2[reset] = -1;
			...
			// uses OptimalSum matrix to find leaves generating the best
			// possible sum of similarities
			if (!stop) {
				...
				// recursive method determines optimal ordering and rotates
				// nodes appropriately
				OptimizeLeafOrder(two_n - 2, NumberOfChildren, LeavesUnder,
						Child1, Child2, OptimalSum, bestU, bestW, n);
			}
		}
		// ========================================
...
//		System.out.println("fillSimMatRow("+i+")");
		if (!optimizeOrdering)
			SimilarityMatrix[i] = new float[i];
		Min[i] = Float.POSITIVE_INFINITY;
		for (int j = 0; j < i; ++j) {
			if (genes) {
				SimilarityMatrix[i][j] = ExperimentUtil.geneDistance(expMatrix,
						null, i, j, function, factor, absolute);// ExpMatrix.GeneDistance(i,j,null);
			} else {
				SimilarityMatrix[i][j] = ExperimentUtil.distance(expMatrix, i,
						j, function, factor, absolute); // ExpMatrix.ExperimentDistance(i,j);
			}
			if (optimizeOrdering) {
				SimilarityMatrix[j][i] = SimilarityMatrix[i][j]; // square
																	// matrix
																	// created
																	// from
				// squareMatrix[j][i] = SimilarityMatrix[i][j]; //triangular
				// Similarity Matrix
			}
			if (SimilarityMatrix[i][j] < Min[i]) {
				Min[i] = SimilarityMatrix[i][j];
		...
		event.setIntValue((int)(((double)i*200)/n));
		fireValueChanged(event);

	}

	/**
	 * AssertParentage takes an unassigned leaf or node (Parent[child] ==-1) and
	 * assigns it to a higher node by fixing the Parent and Child arrays to
	 * reflect the relationship.
	 */
	public void AssertParentage(int[] Parent, int[] NumberOfChildren,
			int[] Child1, int[] Child2, int child, int paren) {
		try {
			if (Parent[child] == -1) {
				Parent[child] = paren;
				--parentless; // global
			...
			fireValueChanged(new AlgorithmEvent(this, AlgorithmEvent.WARNING,
					0, "Error: " + e.toString() + " - AssertParentage("
	...
	public int NewNode(float[] Height, double h) {
		// NewNode creates a node (Assigned) and assigns it an integer value
		// >=n.
	...
	public void MakeLeavesUnderMatrix(int node, int[] Child1, int[] Child2,
			int n, int[][] LeavesUnder, int[] NumberOfChildren) {
		/*
		 ...
		 */
		if (Child1[node] >= n) {
			MakeLeavesUnderMatrix(Child1[node], Child1, Child2, n, LeavesUnder,
					NumberOfChildren);
		...
		if (Child2[node] >= n) {
			MakeLeavesUnderMatrix(Child2[node], Child1, Child2, n, LeavesUnder,
					NumberOfChildren);
	 ...
	 * 
	 * @param Child1
	 *            the first child of a given node
	 * @param Child2
	 *            the second child of a given node
	 * @param NumberOfChildren
	 *            the number of leaves below a given node
	 * @param LeavesUnder
	 *            an array of nodes with arrays of leaves under those nodes
	 * @param OptimalSum
	 *            matrix containing the optimal sum of all similarities between
	 *            adjacent leaves between two leaves set to be a maximum apart
	 * @param SimilarityMatrix
	 *            matrix containing similarities between all leaves
	 * @param n
	 ...
	 * @author dschlauch
	 */
	public void MakeOptimalSumMatrix(int[] Child1, int[] Child2,
			int[] NumberOfChildren, int[][] LeavesUnder, float[][] OptimalSum,
			float[][] SimilarityMatrix, int n, int node, AlgorithmEvent optevent) {
		...
		if (node >= n) {
			...
			if (NumberOfChildren[node] == 2) { // The OptimalSum between the two
												// leaves of a leaf pair is
												// simply their similarity
				...
				MakeOptimalSumMatrix(Child1, Child2, NumberOfChildren,
						LeavesUnder, OptimalSum, SimilarityMatrix, n,
						Child1[node], optevent);
				MakeOptimalSumMatrix(Child1, Child2, NumberOfChildren,
						LeavesUnder, OptimalSum, SimilarityMatrix, n,
						Child2[node], optevent);
				...
				for (int arrangement = 0; arrangement < 4; arrangement++) {
					 ...
					 * of mnode foroptimal ordering of the node "node" with the
					...
					for (int u = 0; u < NumberOfChildren[unode]; u++) {
						...
						for (int align = 0; align < NumberOfChildren[mnode]; align++) {
							leafRankingOrderU[u][align] = LeavesUnder[mnode][Arrays
									.binarySearch(
											rankedU[u],
											OptimalSum[LeavesUnder[unode][u]][LeavesUnder[mnode][align]])];
						}
					}
					for (int w = 0; w < NumberOfChildren[wnode]; w++) {
						...
						for (int align = 0; align < NumberOfChildren[knode]; align++) {
							leafRankingOrderW[w][align] = LeavesUnder[knode][Arrays
									.binarySearch(
											rankedW[w],
											OptimalSum[LeavesUnder[wnode][w]][LeavesUnder[knode][align]])];
						}
					}
					fillOptSumMatrix(NumberOfChildren, rankedU, rankedW,
							SimilarityMatrix, leafRankingOrderU,
							leafRankingOrderW, OptimalSum, LeavesUnder, unode,
							mnode, knode, wnode, optevent);
				}
			}
		}
	}
	 ...
	 * stored in the OptimalSum Matrix.
	 * 
	 * @param NumberOfChildren
	 *            the number of leaves below a given node
	 * @param rankedU
	 *            contains an array in ascending order of the optimal sum values
	 *            for each leaf of U
	 * @param rankedW
	 *            contains an array in ascending order of the optimal sum values
	 *            for each leaf of W
	 * @param SimilarityMatrix
	 *            matrix containing similarities between all leaves
	 * @param leafRankingOrderU
	 *            , leafRankingOrderW contains a ranked order of leaves in M by
	 *            optimal sum versus a leaf in U
	 * @param leafRankingOrderU
	 *            , leafRankingOrderW contains a ranked order of leaves in K by
	 *            optimal sum versus a leaf in W
	 * @param OptimalSum
	 *            matrix containing the optimal sum of all similarities between
	 *            adjacent leaves between two leaves set to be a maximum apart
	 * @param LeavesUnder
	 ...
	 *            the total number of leaves
	 * @param unode
	 *            the grand-child node on the outside, adjacent to mnode
	 * @param mnode
	 *            the grand-child node on the inside, adjacent to unode
	 * @param knode
	 *            the grand-child node on the inside, adjacent to wnode
	 * @param wnode
	 *            the grand-child node on the outside, adjacent to knode
	 * @author dschlauch
	 */
	public void fillOptSumMatrix(int[] NumberOfChildren, float[][] rankedU,
			float[][] rankedW, float[][] SimilarityMatrix,
			int[][] leafRankingOrderU, int[][] leafRankingOrderW,
			float[][] OptimalSum, int[][] LeavesUnder, int unode, int mnode,
			int knode, int wnode, AlgorithmEvent optevent) {
			...
			for (int w = 0; w < NumberOfChildren[wnode]; w++) {
				...
				for (int m = 0; m < NumberOfChildren[mnode]; m++) {
					if (rankedU[u][m] + rankedW[w][0] + bestC >= currentMin)
						break; // halts search when a better arrangement in "m"
								// can no longer be found

					for (int k = 0; k < NumberOfChildren[knode]; k++) {
						if (rankedU[u][m] + rankedW[w][k] + bestC >= currentMin)
							break; // halts search when a better arrangement in
									// in "k" can no longer be found
	 ...
	 *            adjacent leaves between two leaves set to be a maximum apart
	 * @param bestU
	 *            the leaf set to be on the outside of the child U
	 * @param bestW
	 *            the leaf set to be on the outside of the child W
	 * @param n
	 *            the total number of leaves
	 * @author dschlauch
	 */
	private void OptimizeLeafOrder(int node, int[] NumberOfChildren,
			int[][] LeavesUnder, int[] Child1, int[] Child2,
			float[][] OptimalSum, int bestU, int bestW, int n) {
		float match;
		float bestMatch = Float.POSITIVE_INFINITY;
		int bestM = -1;
		int bestK = -1;
		...
		rotateNodes(NumberOfChildren, Child1, Child2, bestU, bestW, node, n,
				LeavesUnder);
		...
		for (int m = 0; m < leftInnerChildren; m++) {
			for (int k = 0; k < rightInnerChildren; k++) {
				...
				match = bestLeft + bestRight
						+ OptimalSum[innerLeftLeaf][innerRightLeaf];
				if (match < bestMatch) {
					bestMatch = match;
					bestM = innerLeftLeaf;
		...
		if (Child1[node] >= n) {
			OptimizeLeafOrder(Child1[node], NumberOfChildren, LeavesUnder,
					Child1, Child2, OptimalSum, bestU, bestM, n);
		}
		if (Child2[node] >= n) {
			OptimizeLeafOrder(Child2[node], NumberOfChildren, LeavesUnder,
					Child1, Child2, OptimalSum, bestK, bestW, n);
		}
	}
	 ...
	 *            the leaf set to be on the outside of the child W
	 * @param node
	 *            the node that will have its children or grand-children
	 ...
	 * @author dschlauch
	 */
	private void rotateNodes(int[] NumberOfChildren, int[] Child1,
